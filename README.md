...
A personal workspace for an AI intern at MCCIA to manage **consulting engagements**, **app development projects**, and **social media creatives** across multiple client companies. Data is stored in **Supabase (Postgres)** via lightweight serverless functions and is **private to each signed-in user** — everyone gets the same features, their own data.
...
`/api/me`, `/api/logout`) against the same Supabase database as production via a
small Vite middleware, so local and deployed behaviour match. Add companies,
sessions, projects, and creatives in the app and they're saved to Supabase. Sign
in with one of the seeded accounts (e.g. `sujal@mcciapune.com`).
...
> Users live in the Supabase database (the `users` table). There are no passwords
...
> accounts in the database (see [`supabase/seed.sql`](supabase/seed.sql)).

## Data & storage

Records live in **Supabase (Postgres)**. Each entity is stored in a single generic
...
Supabase. Its surface (`read / append / update / remove / overwriteMany`, with `id` and
...
exists in the `users` table the server issues a signed session token (an HS256
...
- `server/users.ts` looks users up in Supabase; `server/authApi.ts` has the handlers.
...
- Each user's data is private (scoped by `owner_id` in Supabase). **Settings → Export
